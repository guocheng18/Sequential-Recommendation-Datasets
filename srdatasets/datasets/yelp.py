...
class Yelp(Dataset):
    ...
    def download(self):
        if not self.rootdir.joinpath("yelp_dataset.tar.gz").exists():
            logger.warning(
                "Since Yelp dataset is not directly accessible, please visit \
                    https://www.yelp.com/dataset/download and download it manually, \
                    after downloaded, place file 'yelp_dataset.tar.gz' \
    ...
    def transform(self, stars_threshold):
        df = pd.read_json(self.rootdir.joinpath(self.__corefile__), orient="records", lines=True)
